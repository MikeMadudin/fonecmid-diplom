...
Процедура СформироватьДвиженияПоУдержаниям()
	...
	Для Каждого Строка Из Начисления Цикл
		...
		Движение.БазовыйПериодНачало = НачалоДня(Строка.ДатаНачала);
	...
	Запрос.Текст =
		...
		|	ВКМ_УдержанияБазаВКМ_ОсновныеНачисления.Результат,
		...
		|		ВидРасчета = &ВидРасчета И Регистратор = &Ссылка) КАК ВКМ_УдержанияБазаВКМ_ОсновныеНачисления";
	...
	Пока Выборка.Следующий() Цикл
		...
		Движение.Результат = Выборка.РезультатБаза - (Выборка.РезультатБаза * 0.87);
	КонецЦикла;
