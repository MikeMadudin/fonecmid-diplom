...
Функция СоздатьРеализацииПакетом(Период) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
				   ...
				   |			ПО вт_ДоговорыЗаПериод.Договор = РеализацияТоваровУслуг.Договор
				   |			И (НЕ РеализацияТоваровУслуг.ПометкаУдаления)) КАК Реализации
				   |ГДЕ
	...
	Если Не Результат.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Попытка
				...
				РеализацияСтруктурой = Новый Структура;
				РеализацияСтруктурой.Вставить("Реализация", НоваяРеализация.Ссылка);
				РеализацияСтруктурой.Вставить("Договор", Выборка.Договор);
				СозданныеРеализации.Добавить(РеализацияСтруктурой);
			Исключение
